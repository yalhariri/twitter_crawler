...
if __name__== "__main__":
	...
	if core != None and uname != None and pswrd != None :
		...
		atexit.register(exitHandler, project =f"{PROJECT}-{core}", tool='Sentiment Updater', key=KEY)
		...
		tweets_list = []
		sentiment = dict()
		q='(sentiment:NoneEnglish AND language:*) OR (NOT sentiment:* AND language:*)'
			...
			for tweet in result.docs:
				try:
					if 'language' in tweet.keys():
						...
						if tweet['language'] in supported_languages:
							sentiment[tweet["id"]] = {'id': tweet['id'], "full_text": tweet["full_text"], "language":language_dict_inv[language_]}
						else:
							temp_dict = {'id': tweet['id'], 'sentiment': 'NotSupported', 'language_s':language_}
